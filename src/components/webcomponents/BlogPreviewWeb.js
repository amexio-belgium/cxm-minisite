...
export class BlogPreviewWeb extends HTMLElement {
  ...
  createElement() {
    return `
   <article class="group h-full relative flex flex-col justify-stretch has-[:focus-visible]:rounded-xl has-[:focus-visible]:outline has-[:focus-visible]:outline-4 has-[:focus-visible]:outline-offset-0 has-[:focus-visible]:outline-orange-500">
      ...
      <div class="flex h-full min-w-full flex-col gap-2 border-b-2 border-y-quaternary p-2 py-6">
        <div class="flex flex-row items-center gap-2">
          <formatted-date classes="text-xs font-light" date='${this.publicationDate}' />
        ...
        <h2 class="mb-2 font-serif text-lg font-bold [&>mark]:text-black">
          <a
            href="${this.link}"
            class="to-black to-50% text-black no-underline outline-none content-none before:absolute before:inset-x-0 before:inset-y-0 bg-[length:200%_100%] bg-clip-text bg-[100%] text-transparent decoration-white decoration-1 underline-offset-2 hover:bg-[0%_100%] focus-visible:rounded focus-visible:px-2 focus-visible:outline-none focus-visible:ring-2 motion-reduce:transition-none bg-gradient-to-r from-secondary from-0% via-secondary via-50% transition-all hover:decoration-secondary focus-visible:ring-secondary"
            style="transition-duration: ${stegaClean(this.title).length * 20}ms"
          >
            ${this.title}
          </a>
        </h2>
