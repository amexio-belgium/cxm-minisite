...
export const getHrefFromLinkObject = (linkObject: LinkObjectReferenced|Link|null, language: string, preview: boolean) => {
    let href = preview ? '/preview/' : '';
    if(linkObject?.type === 'internal' && linkObject.internalLink &&
       (linkObject.internalLink?._type == 'contentPage' || linkObject.internalLink._type == 'service') 
    ){
